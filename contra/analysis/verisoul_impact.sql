-- Weekly Cohort Monitoring — Earning & Redemption Rates by Platform x Country
-- Cohort:      All users grouped by signup week (Monday), last ~60 days
...
select customer_id from  `weward-1548152103232.raw_data.auth` where email in 
(
select raw_email from  
`weward-1548152103232.study.verisoul_export_31_03_2026` where decision = 'Fake' 
)
...
(
select raw_email from  
`weward-1548152103232.study.verisoul_export_31_03_2026` where decision = 'Suspicious' 
)
...
cohort_users AS (
  SELECT
   ...
   --    END,
	  ...
	  -- ,
	  CASE
        WHEN b.customer_id is not null THEN 'F'
        ELSE ''
      END
	  , '-'
	  	  ,CASE
        WHEN c.customer_id is not null THEN 'S'
        ELSE ''
      END
    ) AS platform_country
  FROM `weward-1548152103232.silver.customer_data` as a 
  left join verisoul as b 
  on a.customer_id = b.customer_id
  left join verisoul_susp  as c
...
SELECT
  ...
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42
  ...
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 14  THEN SUM(co.cashout_delivered_wards_w0) ELSE NULL END AS cashout_delivered_wards_w0
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 14  THEN SUM(co.cashout_delivered_users_w0) ELSE NULL END AS cashout_delivered_users_w0
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 14  THEN SAFE_DIVIDE(SUM(co.cashout_delivered_users_w0), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_delivered_rate_w0
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 21 THEN SUM(co.cashout_delivered_wards_w1) ELSE NULL END AS cashout_delivered_wards_w1
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 21 THEN SUM(co.cashout_delivered_users_w1) ELSE NULL END AS cashout_delivered_users_w1
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 21 THEN SAFE_DIVIDE(SUM(co.cashout_delivered_users_w1), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_delivered_rate_w1
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 28 THEN SUM(co.cashout_delivered_wards_w2) ELSE NULL END AS cashout_delivered_wards_w2
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 28 THEN SUM(co.cashout_delivered_users_w2) ELSE NULL END AS cashout_delivered_users_w2
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 28 THEN SAFE_DIVIDE(SUM(co.cashout_delivered_users_w2), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_delivered_rate_w2
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 35 THEN SUM(co.cashout_delivered_wards_w3) ELSE NULL END AS cashout_delivered_wards_w3
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 35 THEN SUM(co.cashout_delivered_users_w3) ELSE NULL END AS cashout_delivered_users_w3
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 35 THEN SAFE_DIVIDE(SUM(co.cashout_delivered_users_w3), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_delivered_rate_w3
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42 THEN SUM(co.cashout_delivered_wards_w4) ELSE NULL END AS cashout_delivered_wards_w4
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42 THEN SUM(co.cashout_delivered_users_w4) ELSE NULL END AS cashout_delivered_users_w4
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42 THEN SAFE_DIVIDE(SUM(co.cashout_delivered_users_w4), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_delivered_rate_w4

  -- ===== W0–W4: Cashout — pending =====
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 14  THEN SUM(co.cashout_pending_wards_w0) ELSE NULL END AS cashout_pending_wards_w0
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 14  THEN SUM(co.cashout_pending_users_w0) ELSE NULL END AS cashout_pending_users_w0
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 14  THEN SAFE_DIVIDE(SUM(co.cashout_pending_users_w0), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_pending_rate_w0
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 21 THEN SUM(co.cashout_pending_wards_w1) ELSE NULL END AS cashout_pending_wards_w1
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 21 THEN SUM(co.cashout_pending_users_w1) ELSE NULL END AS cashout_pending_users_w1
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 21 THEN SAFE_DIVIDE(SUM(co.cashout_pending_users_w1), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_pending_rate_w1
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 28 THEN SUM(co.cashout_pending_wards_w2) ELSE NULL END AS cashout_pending_wards_w2
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 28 THEN SUM(co.cashout_pending_users_w2) ELSE NULL END AS cashout_pending_users_w2
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 28 THEN SAFE_DIVIDE(SUM(co.cashout_pending_users_w2), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_pending_rate_w2
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 35 THEN SUM(co.cashout_pending_wards_w3) ELSE NULL END AS cashout_pending_wards_w3
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 35 THEN SUM(co.cashout_pending_users_w3) ELSE NULL END AS cashout_pending_users_w3
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 35 THEN SAFE_DIVIDE(SUM(co.cashout_pending_users_w3), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_pending_rate_w3
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42 THEN SUM(co.cashout_pending_wards_w4) ELSE NULL END AS cashout_pending_wards_w4
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42 THEN SUM(co.cashout_pending_users_w4) ELSE NULL END AS cashout_pending_users_w4
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42 THEN SAFE_DIVIDE(SUM(co.cashout_pending_users_w4), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_pending_rate_w4

  -- ===== W0–W4: Cashout — attempted (all except refunded) =====
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 14  THEN SUM(co.cashout_attempted_wards_w0) ELSE NULL END AS cashout_attempted_wards_w0
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 14  THEN SUM(co.cashout_attempted_users_w0) ELSE NULL END AS cashout_attempted_users_w0
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 14  THEN SAFE_DIVIDE(SUM(co.cashout_attempted_users_w0), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_attempted_rate_w0
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 21 THEN SUM(co.cashout_attempted_wards_w1) ELSE NULL END AS cashout_attempted_wards_w1
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 21 THEN SUM(co.cashout_attempted_users_w1) ELSE NULL END AS cashout_attempted_users_w1
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 21 THEN SAFE_DIVIDE(SUM(co.cashout_attempted_users_w1), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_attempted_rate_w1
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 28 THEN SUM(co.cashout_attempted_wards_w2) ELSE NULL END AS cashout_attempted_wards_w2
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 28 THEN SUM(co.cashout_attempted_users_w2) ELSE NULL END AS cashout_attempted_users_w2
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 28 THEN SAFE_DIVIDE(SUM(co.cashout_attempted_users_w2), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_attempted_rate_w2
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 35 THEN SUM(co.cashout_attempted_wards_w3) ELSE NULL END AS cashout_attempted_wards_w3
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 35 THEN SUM(co.cashout_attempted_users_w3) ELSE NULL END AS cashout_attempted_users_w3
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 35 THEN SAFE_DIVIDE(SUM(co.cashout_attempted_users_w3), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_attempted_rate_w3
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42 THEN SUM(co.cashout_attempted_wards_w4) ELSE NULL END AS cashout_attempted_wards_w4
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42 THEN SUM(co.cashout_attempted_users_w4) ELSE NULL END AS cashout_attempted_users_w4
  ,CASE WHEN DATE_DIFF(CURRENT_DATE, c.cohort_date, DAY) >= 42 THEN SAFE_DIVIDE(SUM(co.cashout_attempted_users_w4), COUNT(DISTINCT c.customer_id)) ELSE NULL END AS cashout_attempted_rate_w4
...
WHERE c.cohort_date >= DATE_TRUNC(DATE_SUB(CURRENT_DATE, INTERVAL 1 YEAR), WEEK(MONDAY))
